...
var inCmd = &cobra.Command{
	...
	Long: `
Use sub in to substitute Octopus variables in the format '#{variable}'
...
octopipe sub in -c -f deploystep1.ps1,deploystep2.ps1 scripts/ DevTest
...
func init() {
	...
	inCmd.Flags().BoolP("check-only", "c", false, "Check only for variables not present in octopipe.yaml (do not sub)")
	inCmd.Flags().StringP("filenames", "f", "", "File names, separated by comma. If not specifed all files in directory are subbed")
}
